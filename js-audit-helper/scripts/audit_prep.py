...
SECURITY_PATTERNS = {
    ...
    'user_input': [
        ...
        (r'document\.(URL|documentURI)', 'Document URL source'),
        ...
        (r'addEventListener\s*\(\s*["\']message["\']', 'postMessage receiver'),
        (r'\.search\s*\(', 'URL search params'),
        (r'URLSearchParams', 'URLSearchParams usage'),
    ],
    'redirects': [
    ...
    'network': [
        ...
        (r'\.ajax\s*\(\s*{[^}]*url\s*:\s*[\'"]http:', 'jQuery AJAX HTTP'),
        (r'credentials\s*:\s*["\']include["\']', 'Credentials included in fetch'),
        (r'cors\s*:\s*["\']no-cors["\']', 'no-cors mode'),
    ],
    'sql_injection': [
        (r'execute\s*\(\s*["\']SELECT.*\+', 'String concatenation in SQL'),
        (r'execute\s*\(\s*f["\']SELECT', 'f-string in SQL query'),
        (r'\.format\s*\(.*SELECT', 'String format in SQL'),
        (r'query\s*\(\s*["\'][^"\']*%s', 'String formatting in query'),
        (r'cursor\.execute\s*\([^?]*\+', 'SQL concatenation'),
        (r'raw\s*\(', 'Raw SQL query'),
        (r'WHERE.*\+', 'Concatenation in WHERE clause'),
    ],
    'command_injection': [
        ...
        (r'eval\s*\(\s*request', 'eval() with request data'),
        (r'__import__\s*\(', 'Dynamic import'),
    ],
    'path_traversal': [
        (r'open\s*\([^)]*\+', 'File path concatenation'),
        (r'file\s*=\s*.*request', 'Request data in file path'),
        (r'\.\./', 'Directory traversal pattern'),
        (r'os\.path\.join\s*\([^)]*request', 'Path join with request data'),
        (r'readFile\s*\([^)]*\+', 'File read with concatenation'),
        (r'fs\.readFileSync\s*\([^)]*\+', 'readFileSync with concatenation'),
    ],
    'xxe': [
        (r'parse\s*\([^)]*XMLParser[^)]*resolve_entities\s*=\s*True', 'XML parser with entities'),
        (r'ElementTree\.parse\s*\(', 'ElementTree.parse()'),
        (r'etree\.fromstring\s*\(', 'etree.fromstring()'),
        (r'<!ENTITY', 'XML entity definition'),
        (r'DOMParser.*parseFromString', 'DOM parser parseFromString'),
    ],
    'ssrf': [
        (r'requests\.(get|post)\s*\([^)]*request', 'HTTP request with user input'),
        (r'urllib\.request\.urlopen\s*\([^)]*request', 'urlopen with user input'),
        (r'fetch\s*\([^)]*req\.', 'Fetch with request data'),
        (r'axios\.(get|post)\s*\([^)]*req\.', 'Axios with request data'),
        (r'http\.get\s*\([^)]*req\.', 'HTTP get with request data'),
    ],
    'deserialization': [
        (r'pickle\.loads\s*\(', 'Pickle deserialization'),
        (r'yaml\.load\s*\([^)]*Loader\s*=\s*yaml\.Loader', 'Unsafe YAML load'),
        (r'JSON\.parse\s*\([^)]*cookie', 'JSON parse from cookie'),
        (r'unserialize\s*\(', 'PHP unserialize'),
        (r'ObjectInputStream', 'Java object deserialization'),
    ],
    'crypto_issues': [
        (r'md5\s*\(', 'MD5 hash usage'),
        (r'sha1\s*\(', 'SHA1 hash usage'),
        (r'Random\s*\(', 'Weak random (use secrets module)'),
        (r'Math\.random\s*\(', 'Math.random for security'),
        (r'ECB', 'ECB mode encryption'),
        (r'DES', 'DES encryption'),
        (r'RC4', 'RC4 encryption'),
    ],
    'authentication': [
        (r'verify\s*=\s*False', 'SSL verification disabled'),
        (r'AUTH.*NONE', 'No authentication'),
        (r'allow[_-]?anonymous', 'Anonymous access allowed'),
        (r'require[_-]?auth.*false', 'Auth requirement disabled'),
        (r'BasicAuth', 'Basic authentication (weak)'),
    ],
}
