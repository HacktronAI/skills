...
If it is unavailable and the user authorized runtime setup, install the current stable HOL Guard 3.0.5 release in an isolated tool environment:

```bash
uv tool install "hol-guard[cisco]==3.0.5"
hol-guard --version
```

Require the version check to report `3.0.5`. Do not silently replace the pin with `latest`, a prerelease, or a branch URL.
