...
If it is unavailable and the user authorized runtime setup, install the pinned stable HOL Guard 3.0.12 release used by this contribution in an isolated tool environment:

```bash
uv tool install "hol-guard[cisco]==3.0.12"
hol-guard --version
```

Require the version check to report `3.0.12`. Do not silently replace the pin with `latest`, a prerelease, or a branch URL.
